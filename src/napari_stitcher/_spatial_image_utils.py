...
def get_world_affine_from_data_affine(
    ...
    if transform_key_fixed is None:
        ...
        phys2world_fixed = np.array(xim_fixed.attrs['transforms'][transform_key_moving])

    if transform_key_moving is None:
        phys2world_moving = np.eye(data_affine.shape[0])
    else:
        phys2world_moving = np.array(xim_moving.attrs['transforms'][transform_key_moving])

    D_to_P_f = np.matmul(
        transform.SimilarityTransform(translation=get_origin_from_xim(xim_moving, asarray=True)).params,
        transform.SimilarityTransform(scale=get_spacing_from_xim(xim_moving, asarray=True)).params,
    )
    ...
    D_to_P_c = np.matmul(
        transform.SimilarityTransform(translation=get_origin_from_xim(xim_fixed, asarray=True)).params,
        transform.SimilarityTransform(scale=get_spacing_from_xim(xim_fixed, asarray=True)).params,
    )
